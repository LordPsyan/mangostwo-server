...
void LoadNpcTele(void)
{
    ...
    if (result)
    {
        outstring_log("SD3: Loading %s, %s and %s...", Table[0], Table[1], Table[2]);
        ...
        outstring_log("");
        outstring_log("SD3: >> Loaded %u npc_teleport.", nbDest);
    } else outstring_log("SD3: WARNING >> Loaded 0 npc_teleport.");
}
