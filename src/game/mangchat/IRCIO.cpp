...
void IRCClient::Send_WoW_Player(Player *plr, string sMsg)
{
    ...
    ChatHandler::BuildChatPacket(data, CHAT_MSG_SYSTEM, sMsg.c_str(), LANG_UNIVERSAL, plr->GetChatTag(), plr->GetObjectGuid());
    ...
    for(HashMapHolder<Player>::MapType::iterator itr = m.begin(); itr != m.end(); ++itr)
    {
        if (itr->second && itr->second->GetSession()->GetPlayer() && itr->second->GetSession()->GetPlayer()->IsInWorld())
        {
            if(ChannelMgr* cMgr = channelMgr(itr->second->GetSession()->GetPlayer()->GetTeam()))
            {
                ...
                {
                    WorldPacket data;
                    ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, IRCcol2WoW(chat.c_str()).c_str(), LANG_UNIVERSAL, itr->second->GetChatTag(), ObjectGuid(), channel);
