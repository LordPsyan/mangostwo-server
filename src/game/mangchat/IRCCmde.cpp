        ...
        if (Player *plr = GetPlayer(_PARAMS[0]))
        {
            ...
            if (_PARAMS[1] == "release")
            {
                ...
                if (result)
                {
                    ...
                    delete result;
                    plr->m_movementInfo.RemoveMovementFlag(MOVEFLAG_ROOT);
            ...
            else
            {
                ...
                plr->m_movementInfo.AddMovementFlag(MOVEFLAG_ROOT);
    ...
    } else
    {
        ...
        if (chr)
        {
            ...
            if (i_oldlvl == i_newlvl)
                CH.ChatHandler::BuildChatPacket(data, CHAT_MSG_SYSTEM, "Your level progress has been reset.", LANG_UNIVERSAL, CHAT_TAG_NONE, guid);
            else
                if (i_oldlvl < i_newlvl)
                {
                    char  temp [32];
                    sprintf(temp, "You have been leveled up (%i)",i_newlvl-i_oldlvl);
                    CH.ChatHandler::BuildChatPacket(data, CHAT_MSG_SYSTEM, temp, LANG_UNIVERSAL, CHAT_TAG_NONE, guid);
                }
                else
                    if (i_oldlvl > i_newlvl)
                    {
